 ...
 */
Route::{$dataTypeL}(
    ':v/right_module/{$name}_route',
    'right_module/:v.controller.{$Name}Controller/{$name}{$DataTypeL}'
);
";
        // 判断文件是否存在
        if(file_exists("../route/right_route_v1_api.php")){
            file_put_contents(
                '../route/right_route_v1_api.php',
                $String, FILE_APPEND
            );
        }
    }
